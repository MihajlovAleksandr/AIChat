...
public class HttpClient {
    ...
    private final PendingCommandDao database = DatabaseManager.getDatabase().pendingCommandDao();
    ...
    public CompletableFuture<HttpCommand> fetchAsync(
        ...
        String jsonBody = body != null ? JsonHelper.Serialize(body) : null;

        String serverUrl = BuildConfig.SERVER_URL;
        Request request = buildRequest(serverUrl + url, method, jsonBody);

        client.newCall(request).enqueue(createCallback(future, url, method, body, isCritical));
    ...
    public CompletableFuture<HttpCommand> uploadAsync(
        ...
        try {
            ...
            Request request = buildMultipartRequest(serverUrl + url, multipartBuilder.build());

            client.newCall(request).enqueue(createCallback(future, url, HTTPMethod.POST, null, false));
            ...
            future.complete(new HttpCommand(CommandOperation.INTERNAL_SERVER_ERROR, errorNode));
        }
    ...
    public CompletableFuture<File> downloadFileAsync(
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                ...
                if (httpCode != 200) {
                    future.completeExceptionally(new IOException("Server returned code: " + httpCode));
                    return;
                }

                String contentDisposition = response.header("Content-Disposition");
                String fileName = extractFileNameFromContentDisposition(contentDisposition);
                if (fileName == null) fileName = fileId.toString();

                Context context = ConnectionSingleton.getInstance()
                ...
                try (okhttp3.ResponseBody responseBody = response.body();
                    ...
                    long totalBytes = responseBody != null ? responseBody.contentLength() : -1;
                    ...
                    if (listener != null) {
                        long safeTotal = totalBytes > 0 ? totalBytes : 1;
                        listener.onProgress(new UploadProgress(fileId, safeTotal, 0));
                    }
                    ...
                    while ((bytesRead = inputStream.read(buffer)) != -1) {
                        ...
                        if (listener != null) {
                            ...
                            long finalTotal = totalBytes > 0 ? totalBytes : finalDownloaded;

                            mainHandler.post(() -> listener.onProgress(
                                    new UploadProgress(fileId, finalTotal, finalDownloaded)
                            ));
                        }
                    }
    ...
    private Callback createCallback(CompletableFuture<HttpCommand> future, String url, HTTPMethod method, @Nullable Object body, boolean isCritical) {
        return new Callback() {
            @Override
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                ...
                Log.e(TAG, "Raw response: " + (raw.isEmpty() ? "[EMPTY]" : raw));
                Log.e(TAG, "====================");
    ...
    private Request buildMultipartRequest(String url, MultipartBody body) {
        ...
        String language = getAcceptLanguage();
    ...
    private Request buildRequest(String url, HTTPMethod method, @Nullable String jsonBody) {
        ...
        switch (method) {
            ...
            case POST:
                builder.post(okhttp3.RequestBody.create(jsonBody != null ? jsonBody : "{}", JSON_MEDIA_TYPE));
                builder.addHeader("Content-Type", "application/json");
                break;

            case PUT:
                builder.put(okhttp3.RequestBody.create(jsonBody != null ? jsonBody : "{}", JSON_MEDIA_TYPE));
                builder.addHeader("Content-Type", "application/json");
                break;

            case DELETE:
                if (jsonBody != null && !jsonBody.trim().isEmpty()) {
                    builder.delete(okhttp3.RequestBody.create(jsonBody, JSON_MEDIA_TYPE));
                    builder.addHeader("Content-Type", "application/json");
                } else {
                    builder.delete();
                }
    ...
    public void onConnected() {
        List<PendingCommand> commandList = database.getAllCommands();

        for (PendingCommand command : commandList) {
            Object body = command.getCommandFormat(Object.class);
    ...
    private String extractFileNameFromContentDisposition(String contentDisposition) {
        ...
        // Ищем filename*=UTF-8'' (приоритет)
        int utf8Start = contentDisposition.indexOf("filename*=UTF-8''");
        if (utf8Start != -1) {
            int start = utf8Start + "filename*=UTF-8''".length();
