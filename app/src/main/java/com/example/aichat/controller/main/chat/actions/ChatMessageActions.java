...
public class ChatMessageActions {
        ...
        dispatcher.sendHttpRequestAsync("/api/messages", HttpClient.HTTPMethod.POST, request, true)
                .thenAccept(cmd -> {
                    if (cmd.isSuccess()) {
                        ...
                        saver.deleteMessage(message.getId());
                        showErrorToast();
                    }
                })
    ...
    public void onMessageStatusUpdated(UpdateMessageStatusResponse status) {
        if (status == null || !status.chatId.equals(chatId)) return;

        new Thread(() -> {
            saver.updateMessageStatus(status);
            ...
            if (fragment.isAdded() && fragment.getActivity() != null) {
                fragment.getActivity().runOnUiThread(() -> {
                    ...
                    notifyChatsListMessageStatuses(
                            status.messageIds,
                            status.status,
                            true
                    );
    ...
    public void readMessages(List<Message> messages) {
        ...
        new Thread(() -> {
            ...
            try {
                dispatcher.sendSignalRRequestAsync(
                        "UpdateMessageStatus",
    ...
    private void notifyChatsListMessageStatuses(
        ...
        fragment.getActivity().runOnUiThread(() -> {
            ...
            for (UUID messageId : messageIds) {
                activity.updateChatMessageStatusInList(
                        messageId,
                        chatId,
                        status
                );
            ...
            if (refreshAfter) {
                activity.refreshChatsListUnreadState();
            }
        });
    }

    private void refreshChatsListUnreadState() {
        if (!fragment.isAdded() || fragment.getActivity() == null) {
            return;
        }
